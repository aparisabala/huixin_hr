...
class ValidateUpdateEmployeeDocument extends FormRequest
{
   /**
    ...
    {
        $rules =  [
            'doc' => 'nullable|file|mimes:png,jpg,pdf,docx,doc|max:2024'
        ];
        if($row->isDirty('lib_document_id')) {
            $rules['lib_document_id'] =  [
                'required',
                'string',
                'max:100',
