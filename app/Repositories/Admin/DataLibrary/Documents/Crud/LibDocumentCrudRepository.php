...
use App\Http\Requests\Admin\DataLibrary\Documents\Crud\ValidateUpdateLibDocument;
...
use Auth;
use DB;
class  LibDocumentCrudRepository extends BaseRepository implements ILibDocumentCrudRepository {

    use BaseTrait;
    public function __construct() {
        $this->LoadModels(['LibDocument']);
    }
    ...
    {
       return $this->getPageDefault(model: $this->LibDocument, id: $id);
    }
    ...
    {
        $model = LibDocument::query();
        $this->saveTractAction(
            $this->getTrackData(
                title: 'LibDocument was viewed by '.$request?->auth?->name.' at '.Carbon::now()->format('d M Y H:i:s A'),
                request: $request,
                onlyTitle: true
            )
        );
    ...
    {   
        ...
        try {
            LibDocument::create([
                ...$request->all(),
                //'serial' => $this->facSrWc($this->LibDocument)
            ]);
            $response['extraData'] = ['inflate' => pxLang($request->lang,'','common.action_success') ];
            $this->saveTractAction($this->getTrackData(title: "LibDocument was created by ".$request?->auth?->name,request: $request));
            DB::commit();
            return $this->response(['type' => 'success', 'data' => $response]);
        } catch (\Exception $e) {
            DB::rollback();
            $this->saveError($this->getSystemError(['name' => 'LibDocument_store_error']), $e);
    ...
    public function update($request,$id) : JsonResponse
    {
        ...
        if($row->isDirty()){
            $validator = Validator::make($request->all(), (new ValidateUpdateLibDocument())->rules($request,$row));
            if ($validator->fails()) {
            ...
            try {
                ...
                $this->saveTractAction($this->getTrackData(title: " LibDocument ".$row?->name.' was updated by '.$request?->auth?->name,request: $request, row: $rowRef, type: 'to'));
                DB::commit();
                return $this->response(['type' => 'success','data' => $data]);
            } catch (\Exception $e) {
                DB::rollback();
                $this->saveError($this->getSystemError(['name'=>'LibDocument_update_error']), $e);
    ...
    {
        $i = LibDocument::whereIn('id',$request->ids)->select(['id','name'])->get();;
        $dirty = [];
        if (count($i) > 0) {
            ...
            if (count($dirty) > 0) {
                ...
                try {
                    ...
                    $this->saveTractAction($this->getTrackData(title: "LibDocument list was updated by ".$request?->auth?->name, request: $request));
                    DB::commit();
                    return $this->response(['type' => 'success','data' => $data]);
                } catch (\Exception $e) {
                    DB::rollback();
                    $this->saveError($this->getSystemError(['name' => 'LibDocument_bulk_update_error']), $e);
                    return $this->response(['type' => 'wrong', 'lang' => 'server_wrong']);
                }
            } else {
                return $this->response(['type' => 'noUpdate', 'title' =>  '<span class="text-success"> '.pxLang($request->lang,'','common.no_change').'  </span>']);
    ...
    {
        $errors = [];
        $i = LibDocument::whereIn('id',$request->ids)->select(['id'])->get();
        if (count($i) > 0) {
            ...
            try {
                ...
                $this->saveTractAction($this->getTrackData(title: "LibDocument list was deleted by ".$request?->auth?->name, request: $request));
                DB::commit();
                return $this->response(['type' => 'success',"data"=>$data]);
            } catch (\Exception $e) {
                DB::rollback();
