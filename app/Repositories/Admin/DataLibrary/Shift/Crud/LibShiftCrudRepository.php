...
namespace App\Repositories\Admin\DataLibrary\Shift\Crud;

use App\Http\Requests\Admin\DataLibrary\Shift\Crud\ValidateUpdateLibShift;
...
use Auth;
use DB;
class  LibShiftCrudRepository extends BaseRepository implements ILibShiftCrudRepository {

    use BaseTrait;
    public function __construct() {
        $this->LoadModels(['LibShift']);
    }
    ...
    {
       return $this->getPageDefault(model: $this->LibShift, id: $id);
    }
    ...
    {
        $model = LibShift::query();
        $this->saveTractAction(
            $this->getTrackData(
                title: 'LibShift was viewed by '.$request?->auth?->name.' at '.Carbon::now()->format('d M Y H:i:s A'),
                request: $request,
                onlyTitle: true
            )
        );
    ...
    {   
        ...
        try {
            LibShift::create([
                ...$request->all(),
                //'serial' => $this->facSrWc($this->LibShift)
            ]);
            $response['extraData'] = ['inflate' => pxLang($request->lang,'','common.action_success') ];
            $this->saveTractAction($this->getTrackData(title: "LibShift was created by ".$request?->auth?->name,request: $request));
            DB::commit();
            return $this->response(['type' => 'success', 'data' => $response]);
        } catch (\Exception $e) {
            DB::rollback();
            $this->saveError($this->getSystemError(['name' => 'LibShift_store_error']), $e);
    ...
    public function update($request,$id) : JsonResponse
    {
        ...
        if($row->isDirty()){
            $validator = Validator::make($request->all(), (new ValidateUpdateLibShift())->rules($request,$row));
            if ($validator->fails()) {
            ...
            try {
                ...
                $this->saveTractAction($this->getTrackData(title: " LibShift ".$row?->name.' was updated by '.$request?->auth?->name,request: $request, row: $rowRef, type: 'to'));
                DB::commit();
                return $this->response(['type' => 'success','data' => $data]);
            } catch (\Exception $e) {
                DB::rollback();
                $this->saveError($this->getSystemError(['name'=>'LibShift_update_error']), $e);
    ...
    {
        $i = LibShift::whereIn('id',$request->ids)->select(['id','name'])->get();;
        $dirty = [];
        if (count($i) > 0) {
            ...
            if (count($dirty) > 0) {
                ...
                try {
                    ...
                    $this->saveTractAction($this->getTrackData(title: "LibShift list was updated by ".$request?->auth?->name, request: $request));
                    DB::commit();
                    return $this->response(['type' => 'success','data' => $data]);
                } catch (\Exception $e) {
                    DB::rollback();
                    $this->saveError($this->getSystemError(['name' => 'LibShift_bulk_update_error']), $e);
                    return $this->response(['type' => 'wrong', 'lang' => 'server_wrong']);
                }
            } else {
                return $this->response(['type' => 'noUpdate', 'title' =>  '<span class="text-success"> '.pxLang($request->lang,'','common.no_change').'  </span>']);
    ...
    {
        $errors = [];
        $i = LibShift::whereIn('id',$request->ids)->select(['id'])->get();
        if (count($i) > 0) {
            ...
            try {
                ...
                $this->saveTractAction($this->getTrackData(title: "LibShift list was deleted by ".$request?->auth?->name, request: $request));
                DB::commit();
                return $this->response(['type' => 'success',"data"=>$data]);
            } catch (\Exception $e) {
                DB::rollback();
